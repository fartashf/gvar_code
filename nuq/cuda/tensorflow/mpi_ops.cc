...
float *NUQSGDdequan_buffer;
curandState* cuda_states;
std::vector<float*> norm_send;
std::vector<float*> norm_recv;
std::vector<signed char*> NUQSGDquantizedGradients;
std::vector<signed char*> NUQSGDquantizedGradients_recv;
...
void PerformOperation(TensorTable& tensor_table, MPIResponse response) {
  ...
  } else if (response.response_type() == MPIResponse::ALLREDUCE) {//sd_allreduce
...
// #endif
   ...
   MPI_Datatype dtype;
    ...
    if(NUQSGDdequan_buffer == nullptr)
      cudaMalloc(&NUQSGDdequan_buffer, chunk_size); //size of dequan buffer is the max size for one chunk
    ...
    if(NUQSGDquantizedGradients.size() != (numNodes - 1))
    {
      ...
      for(int i = 0; i < (numNodes - 1); i++)
      {
        ...
        float *normPerNode;
        float *normPerNode_recv; 
        signed char *NUQSGDquantizedGradientsPerNode;
        signed char *NUQSGDquantizedGradientsPerNode_recv;

        //calculate how many buckets in each chunk. Then each bucket has 1 float value: norm.
        cudaMalloc(&normPerNode, ceil((double)chunk_size / (bucket_size * sizeof(float))) * sizeof(float));
        cudaMalloc(&normPerNode_recv, ceil((double)chunk_size / (bucket_size * sizeof(float))) * sizeof(float));
        
        //We quantize values from 32 bits to 8 bits, so the size of quantized chunk is 1/4 of full precision chunk.
        cudaMalloc(&NUQSGDquantizedGradientsPerNode, ceil((double)chunk_size / 4.0));
        cudaMalloc(&NUQSGDquantizedGradientsPerNode_recv, ceil((double)chunk_size / 4.0));

        norm_send.push_back(normPerNode); 
        NUQSGDquantizedGradients.push_back(NUQSGDquantizedGradientsPerNode);
        norm_recv.push_back(normPerNode_recv);
        NUQSGDquantizedGradients_recv.push_back(NUQSGDquantizedGradientsPerNode_recv);
      }
    ...
    for(int division = 0; division < num_divisions; division++)
    {
      ...
      for(int i = 0; i < numNodes; i++)
      {
        if(i != rank) //for each node(chunk)
        {
          ...
          MPI_Irecv(NUQSGDquantizedGradients_recv[counter1], numElems, MPI_UNSIGNED_CHAR, i, 0, 
            MPI_COMM_WORLD, &request_reduce.back());

          request_reduce.push_back(MPI_Request());
          MPI_Irecv(norm_recv[counter1], numBuckets * 2, dtype, i, 0, MPI_COMM_WORLD, &request_reduce.back());

          //find norm of this chunk
       
          GPUfindNorm2((float*)buffer_data + division_offset + start_offset, norm_send[counter1], 
            length, horovod_global.streams[first_entry.device]);
          
          //quantize this chunk
          GPUNUQSGDquantizeValue(NUQSGDquantizedGradients[counter1], (float*)buffer_data + division_offset + start_offset, 
            norm_send[counter1], length, cuda_states, horovod_global.streams[first_entry.device]);
          
          request_reduce.push_back(MPI_Request());
          MPI_Isend(NUQSGDquantizedGradients[counter1], numElemsPerNode + ((i < residue) ? 1 : 0), 
            MPI_UNSIGNED_CHAR, i, 0, MPI_COMM_WORLD, &request_reduce.back());

          request_reduce.push_back(MPI_Request());
          MPI_Isend(norm_send[counter1], numBuckets * 2, dtype, i, 0, MPI_COMM_WORLD, &request_reduce.back());
      ...
      {
        //dequantization
        GPUNUQSGDdequantizeValue(NUQSGDquantizedGradients_recv[i], norm_recv[i], 
          NUQSGDdequan_buffer, numElems, horovod_global.streams[first_entry.device]);
        
        //add dequantized value to right place of data_buffer 
        GPUAdd(numElems, NUQSGDdequan_buffer, (float*)buffer_data + division_offset + startElem, 
          horovod_global.streams[first_entry.device]); 
      }
      
      //quantize aggregated value  
      
      GPUfindNorm2((float*)buffer_data + division_offset + startElem, norm_recv[0], numElems, 
        horovod_global.streams[first_entry.device]);

      GPUNUQSGDquantizeValue(NUQSGDquantizedGradients_recv[0], (float*)buffer_data + division_offset + startElem, 
        norm_recv[0], numElems, cuda_states, horovod_global.streams[first_entry.device]);
      ...
      for(int i = 0; i < numNodes; i++)
      {
        if (i != rank)
        {
          ...
          MPI_Irecv(NUQSGDquantizedGradients[counter2], numElemsPerNode + ((i < residue) ? 1 : 0), 
            MPI_UNSIGNED_CHAR, i, 0, MPI_COMM_WORLD, &request_gather.back());

          request_gather.push_back(MPI_Request());
          MPI_Irecv(norm_send[counter2], numBuckets * 2, dtype, i, 0, MPI_COMM_WORLD, &request_gather.back());  

          request_gather.push_back(MPI_Request());
          MPI_Isend(NUQSGDquantizedGradients_recv[0], numElems, MPI_UNSIGNED_CHAR, i, 0, MPI_COMM_WORLD, &request_gather.back());

          request_gather.push_back(MPI_Request());
          MPI_Isend(norm_recv[0], numBuckets * 2, dtype, i, 0, MPI_COMM_WORLD, &request_gather.back());      
      ...
      for(int i = 0; i < numNodes; i++)
      {
        ...
        if(i != rank)
        {
          ...
          GPUNUQSGDdequantizeValue(NUQSGDquantizedGradients[counter3], norm_send[counter3], 
            NUQSGDdequan_buffer, length, horovod_global.streams[first_entry.device]);
          
          //copy dequantized data to right place of data_buffer
          GPUCopyValue((float*)buffer_data + division_offset + start_offset, NUQSGDdequan_buffer, length, 
            horovod_global.streams[first_entry.device]);
        ...
        {
          //dequantize chunk from node i in dequan_buffer  NUQSGDquantizedGradients_recv   
          GPUNUQSGDdequantizeValue(NUQSGDquantizedGradients_recv[0], norm_recv[0], 
            NUQSGDdequan_buffer, length, horovod_global.streams[first_entry.device]);
...
//   extern template GPUScaleAndAdd;              \
//   REGISTER_KERNEL_BUILDER(                                       \
//       Name("GPUScaleAndAdd").Device(DEVICE_GPU).TypeConstraint<T>("T"), \
