...
#     grads, _ = tf.clip_by_global_norm(gs, config.max_grad_norm)
    ...
    ##############
    gs_FisherNoise = [g + FishNoise for g, FishNoise in zip(gs, fvps)]
    # gs_FisherNoise2, _ = tf.clip_by_global_norm(gs_FisherNoise, config.max_grad_norm)
    ...
    if FLAGS.useSGD:
      ...
      self.optimizer = tf.train.GradientDescentOptimizer(self._lr)
      self._train_op = self.optimizer.apply_gradients(
          zip(gs_FisherNoise2, tvars),
          global_step=tf.contrib.framework.get_or_create_global_step())

    elif FLAGS.useAdam_diagF:
      self.optimizer = tf.train.AdamOptimizer(self._lr*0.001)
      self._train_op = self.optimizer.apply_gradients(
          zip(gs_FisherNoise, tvars),
          global_step=tf.contrib.framework.get_or_create_global_step())
      # gs_FisherNoise, _ = tf.clip_by_global_norm(gs_FisherNoise, config.max_grad_norm)
