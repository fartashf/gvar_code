...
flags.DEFINE_boolean('useAsync', False,"""use asyn KFAC optimizer""")
flags.DEFINE_boolean('factoredDamping', False,"""use factored damping""")
flags.DEFINE_integer('kfacUpdate', 3,"""kfac update interval""")
flags.DEFINE_float('lr', 0.1,"""learning rate""")
flags.DEFINE_float('eps', 0.01,"""damping factor""")
flags.DEFINE_float('kl_clip', 10.,"""clipping Fisher norm""")
flags.DEFINE_float('stats_decay', 0.99,"""stats decay""")
flags.DEFINE_integer('cold_iter', 50,"""cold iteration""")
flags.DEFINE_float('cold_lr', 0.0003,"""cold learning rate""")
flags.DEFINE_boolean('debug', False,"""turn on debug flag""")
flags.DEFINE_boolean('temporalDep', False,"""use RNN approximation""")
flags.DEFINE_string('temporalDepOpt', 'option3',"""use option3 RNN approximation""")
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    noise_shape = tf.concat(0, [tf.shape(logits), [1,]])
    ...
    sampled_loss = tf.nn.seq2seq.sequence_loss_by_example(
        [logits],
        [sampled_labels],
        [tf.ones([batch_size * num_steps], dtype=data_type())])
    ...
    if FLAGS.useSGD:
      ...
      self.optimizer = kfac.KfacOptimizer(learning_rate=self._lr*FLAGS.lr, 
                                          cold_lr=FLAGS.cold_lr, 
                                          momentum=0.9, clip_kl=FLAGS.kl_clip,
                                          epsilon=FLAGS.eps, stats_decay=FLAGS.stats_decay, 
                                          blockdiag_bias=False, async=FLAGS.useAsync, 
                                          cold_iter=FLAGS.cold_iter, 
                                          approxT2 = False, 
                                          factored_damping=FLAGS.factoredDamping,
                                          temporal_dependency=FLAGS.temporalDep, 
                                          temporal_dependency_option=FLAGS.temporalDepOpt, 
                                          kfac_update = FLAGS.kfacUpdate, )
      update_stats_op = self.optimizer.compute_and_apply_stats(sampled_cost, var_list=tvars)
      self._train_op, self._QR = self.optimizer.apply_gradients(zip(grads, tvars),)
