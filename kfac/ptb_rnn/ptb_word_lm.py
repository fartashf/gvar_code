...
flags.DEFINE_integer('dataset_size', None, 'dataset_size')
flags.DEFINE_boolean('useSGD', False,"""use SGD optimizer""")
flags.DEFINE_boolean('useAdam', False,"""use Adam optimizer""")
# flags.DEFINE_integer("batch_size", 20, "The batch size used during
        # training.")
FLAGS = flags.FLAGS
BASIC = "basic"
CUDNN = "cudnn"
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    # sv = tf.train.Supervisor(logdir=FLAGS.save_path,save_model_secs=3)
    ...
    with sv.managed_session(config=config_proto) as session:
        ...
        # sv.summary_computed(session, session.run(summ), global_step=i)
#         with open('FLAGS.saved_path', 'a+') as f# :
            with open('/home/kevin/PTBIterationsResults/' + FLAGS.saved_path + '.csv' ,'a+') as f:
