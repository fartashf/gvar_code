...
tf.app.flags.DEFINE_string('name', 'est_debug',"""experiment name""")
...
tf.app.flags.DEFINE_float('noise_lambda', 1.0,"""Noise Lambda""")
...
MOM = FLAGS.mom
EPSILON = FLAGS.eps
NOISE_LAMBDA = FLAGS.noise_lambda
...
with tf.device('/gpu'):
   ...
   gs = tf.gradients(loss, var, name='gradientsSampled') ## HACK: have to use this name!!!
   noise = [tf.random_normal(tf.shape(v)) for v in var]


   ############## estimate curvature (Fisher)
   cov = cov_estimator.Estimator(approx_option='diag') 
   cov.compute_stats(loss, var, gs)
   fvps = cov.FisherVectorProduct(zip(var,noise), power = 0.5, noise_lambda=NOISE_LAMBDA)
   print(fvps)
   ##############
   
   #### Add the Fisher noise into the gradients
   gs_with_noise = [g + f_noise for g, f_noise in zip(gs, fvps)]
   ####
  
   if FLAGS.useAdam:
     updates = tf.train.AdamOptimizer(learning_rate=LR).apply_gradients(zip(gs_with_noise, var))
   else:
     updates = tf.train.MomentumOptimizer(learning_rate=LR, momentum=MOM,).apply_gradients(zip(gs_with_noise, var))
