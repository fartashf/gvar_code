...
def buckets_vs_ngpus(args):
    """cifar10_first_epoch
    """
    dataset = 'cifar10'
    module_name = 'main.gvar'
    log_dir = 'runs_%s_buckets_vs_ngpus' % dataset
    exclude = ['dataset', 'epochs', 'lr_decay_epoch', 'g_epoch',
               'pretrained', 'niters', 'epoch_iters',
               'gvar_log_iter', 'gvar_start', 'g_bsnap_iter', 'g_optim',
               'g_optim_start']
    ...
    args_nuq = [
        ('g_estim', ['nuq']),
        ('nuq_bits', 4),
        ('nuq_bucket_size', [2**10, 2**14, 2**18]),
        ('nuq_ngpu', [2, 4, 8]),
        ('nuq_method', ['q', 'qinf',
                        ('nuq', OrderedDict([('nuq_mul', 0.5)]))
                        ])
...
def imagenet_full_n(args):
    dataset = 'imagenet'
    module_name = 'main.gvar'
    log_dir = 'runs_%s_full_n' % dataset
    exclude = ['dataset', 'epochs', 'lr_decay_epoch', 'g_epoch',
               'pretrained', 'niters', 'epoch_iters',
               'gvar_log_iter', 'gvar_start', 'g_bsnap_iter', 'g_optim',
               'g_optim_start']
