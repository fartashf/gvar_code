...
class BasicLSTMCell_block(RNNCell):
  """Basic LSTM recurrent network cell.
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
      ...
      if self._state_is_tuple:
        ...
        c, h = array_ops.split(state, 2,  1,)
      with vs.variable_scope("InputGates"):
        i = _linear([inputs, h], self._num_units, True)
      with vs.variable_scope("Candidiate"):
        j = _linear([inputs, h], self._num_units, True)
      with vs.variable_scope("ForgetGates"):
        f = _linear([inputs, h], self._num_units, True)
      with vs.variable_scope("OuptutGates"):
        o = _linear([inputs, h], self._num_units, True)
