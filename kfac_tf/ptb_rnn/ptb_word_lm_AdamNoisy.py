...
flags.DEFINE_boolean('useAdamNoisy', False, """use Adam Noisy""")
...
flags.DEFINE_float('learning_rate', 0.001, "learning rate chosen")
flags.DEFINE_float('noise_level', None, """Gaussian noise level""")
FLAGS = flags.FLAGS
...
#     grads, _ = tf.clip_by_global_norm(gs, config.max_grad_norm)
    # noisy_grads = [g + (FLAGS.noise_level)*tf.random_normal(tf.shape(g)) for g in grads]
    ...
    fvps = cov.FisherVectorProduct(zip(tvars,noise), power = 0.5, fisher_noise = FLAGS.fisher_noise)
    #print(fvps)
    ##############
    ...
    if FLAGS.useSGD:
      ...
      self._train_op = self.optimizer.apply_gradients(
          zip(grads, tvars),
          global_step=tf.contrib.framework.get_or_create_global_step())
    elif FLAGS.useAdamNoisy:
      self.optimizer = AdamNoisyOptimizer(self._lr)
      self._train_op = self.optimizer.apply_gradients(
          zip(gs, tvars), fvps,
          global_step=tf.contrib.framework.get_or_create_global_step())
